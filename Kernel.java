// (Sólo difiero respecto a la versión previa en las llamadas a PageFault.replacePage: ahora se pasa 'this' como parámetro Kernel)
// Asegúrate de sustituir tu Kernel.java por esta versión completa si aún no lo hiciste.
// Aquí incluyo la versión adaptada con las dos llamadas actualizadas.
...
public class Kernel extends Thread {
    ...
    // New: physical frames mapping helper
    private int[] physToVirt = null; // index by physical frame -> virtual page id or -1
    private int physFrameCount = 0;
    ...
    public void init(String commands, String config) {
        ...
        if ( config != null ) {
            ...
            // Initialize physical frame mapping array based on formula used across the code
            physFrameCount = (virtPageNum + 1) / 2; // number of physical frames
            physToVirt = new int[physFrameCount];
            for (i = 0; i < physFrameCount; i++) physToVirt[i] = -1;
                ...
                while ((line = in.readLine()) != null) {
                    if (line.startsWith("memset")) {
                        ...
                        while (st.hasMoreTokens()) {
                            ...
                            // Assign physical if provided and update physToVirt safely
                            page.physical = physical;
                            if (physical >= 0) {
                                if (physical < physFrameCount) {
                                    if (physToVirt[physical] != -1) {
                                        System.out.println("MemoryManagement: Duplicate physical page's in " + config);
                                        System.exit(-1);
                                    }
                                    physToVirt[physical] = id;
                                    map_count++;
                                } else {
                                    System.out.println("MemoryManagement: Invalid physical frame in " + config);
                                    System.exit(-1);
                                }
                            }
                        }
        ...
        if ( map_count < ( virtPageNum +1 ) / 2 ) {
            // Assign free physical frames sequentially to unmapped pages until we reach physFrameCount
            int nextFrame = 0;
            for (i = 0; i < virtPageNum; i++) {
                Page page = (Page) memVector.elementAt(i);
                if ( page.physical == -1 && map_count < physFrameCount ) {
                    // find next available frame
                    while (nextFrame < physFrameCount && physToVirt[nextFrame] != -1) nextFrame++;
                    if (nextFrame >= physFrameCount) break;
                    page.physical = nextFrame;
                    physToVirt[nextFrame] = page.id;
                    map_count++;
                    nextFrame++;
                }
            }
    ...
    public int getPhysicalFrameCount() {
        return physFrameCount;
    }

    public synchronized void mapFrame(int frame, int virtPage) {
        if (physToVirt == null) return;
        if (frame < 0 || frame >= physFrameCount) return;
        // Unmap any existing mapping for this frame first
        if (physToVirt[frame] != -1) {
            int prevVirt = physToVirt[frame];
            if (prevVirt >= 0 && prevVirt < memVector.size()) {
                Page prev = (Page) memVector.elementAt(prevVirt);
                if (prev != null) prev.physical = -1;
            }
        }
        physToVirt[frame] = virtPage;
        if (virtPage >= 0 && virtPage < memVector.size()) {
            Page p = (Page) memVector.elementAt(virtPage);
            if (p != null) p.physical = frame;
            if (controlPanel != null) controlPanel.addPhysicalPage(virtPage, frame);
        }
    }

    public synchronized void unmapFrame(int frame) {
        if (physToVirt == null) return;
        if (frame < 0 || frame >= physFrameCount) return;
        int virt = physToVirt[frame];
        physToVirt[frame] = -1;
        if (virt >= 0 && virt < memVector.size()) {
            Page p = (Page) memVector.elementAt(virt);
            if (p != null) p.physical = -1;
            if (controlPanel != null) controlPanel.removePhysicalPage(frame);
        }
    }

    public synchronized int getVirtForFrame(int frame) {
        if (physToVirt == null) return -1;
        if (frame < 0 || frame >= physFrameCount) return -1;
        return physToVirt[frame];
    }

    public synchronized int getFrameForVirt(int virtPage) {
        if (memVector == null) return -1;
        if (virtPage < 0 || virtPage >= memVector.size()) return -1;
        Page p = (Page) memVector.elementAt(virtPage);
        return (p == null) ? -1 : p.physical;
    }
    ...
    public void step() {
        ...
        if (instruct.isRange()) {
            ...
            for (int p = pageStart; p <= pageEnd; p++) {
                ...
                if (wasNotPresent) {
                    // <-- Cambio: pasar 'this' como referencia a Kernel
                    PageFault.replacePage(memVector, virtPageNum, p, controlPanel, this);
            ...
            if (wasNotPresent) {
                // <-- Cambio: pasar 'this' como referencia a Kernel
                PageFault.replacePage(memVector, virtPageNum, pageStart, controlPanel, this);
                if (controlPanel != null) controlPanel.pageFaultValueLabel.setText("YES");
            } else {
                if (controlPanel != null) controlPanel.pageFaultValueLabel.setText("NO");
            }
